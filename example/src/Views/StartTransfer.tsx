...
import { startTransferAspera } from './sdk-code';
...
export default function StartTransfer() {
  ...
  const startTransfer = (): void => {
    try {
      startTransferAspera(JSON.parse(transferSpec));
    } catch (error) {
  ...
  return (
    <div className="example-pages">
      <h2>Code example</h2>
      <CodeSnippet type="multi" feedback="Copied to clipboard" maxCollapsedNumberOfRows={25}>{startTransferAspera.toString()}</CodeSnippet>
      <h2>Try it out</h2>
